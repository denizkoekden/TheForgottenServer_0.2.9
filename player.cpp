...
Creature()
{
	...
	ruleViolationOpen = false;
	ruleViolationAnswered = false;
	ruleViolationClosed = true;
	ruleViolationName = "";
	ruleViolationReport = "";
	ruleViolationTime = 0;
	#endif
...
int32_t Player::getDefense() const
{
	...
	if(shield)
	{
		defense += shield->getDefense() + extraDefense;
		defenseSkill = getSkill(SKILL_SHIELD, SKILL_LEVEL);
	}
	
	if(vocation->defenseMultipler != 1.0)
		defense = int32_t(defense * vocation->defenseMultipler);
...
void Player::addStorageValue(const uint32_t key, const uint64_t value)
{
...
bool Player::getStorageValue(const uint32_t key, uint64_t& value) const
{
...
void Player::sendPing(uint32_t interval)
{
	...
	if(!hasCondition(CONDITION_INFIGHT) && !mayNotLogout)
	{
		if(!client)
...
void Player::onCreatureAppear(const Creature* creature, bool isLogin)
{
	if(isLogin && creature == this)
	{
		...
		#ifndef __CONSOLE__
		gui.pBox.addPlayer(this);
		#endif
		std::cout << name << " has logged in." << std::endl;
		g_game.checkPlayersRecord();

		uint64_t value;
		getStorageValue(STORAGEVALUE_SLEEPING, value);
		if(value != 0)
		{
			uint64_t positionX, positionY, positionZ;
			getStorageValue(STORAGEVALUE_SLEEPINGPOS_X, positionX);
			getStorageValue(STORAGEVALUE_SLEEPINGPOS_Y, positionY);
			getStorageValue(STORAGEVALUE_SLEEPINGPOS_Z, positionZ);
			Tile* tile = g_game.getTile(positionX, positionY, positionZ);
			if(tile)
			{
				...
				if(item)
				{
					if(item->isBed())
					{
						uint64_t minutesSleeping = 0;
						if(item->getSleeper() == guid)
						{
							minutesSleeping = (time(NULL) - lastLogout) / 60;
							item->resetSleeper();
							item->transformTakenBedIntoFree(Position(positionX, positionY, positionZ));
						}
						else
							minutesSleeping = (value - lastLogout) / 60;

						if(minutesSleeping >= 2)
						{
							changeHealth((int32_t)(minutesSleeping / 2));
							changeMana((int32_t)(minutesSleeping / 2));
						}

						if(minutesSleeping >= 7)
							changeSoul((int32_t)(minutesSleeping / 7));

						for(int32_t i = STORAGEVALUE_SLEEPING; i <= STORAGEVALUE_SLEEPINGPOS_Z; i++)
							addStorageValue(i, 0);
					}
				}
			}
...
void Player::onCreatureDisappear(const Creature* creature, uint32_t stackpos, bool isLogout)
{
	...
	if(creature == this)
	{
		...
		#ifdef __RULEVIOLATIONREPORT__
		if(client)
			client->removeRuleViolation(name);
		g_game.hasRuleViolationsChannelOpen.remove(this);
		#endif
		#ifndef __CONSOLE__
		gui.pBox.removePlayer(this);
		#endif
...
void Player::onThink(uint32_t interval)
{
	...
	if(!mayNotLogout && !mayNotMove && !isAccessPlayer())
	{
		idleTime += interval;
	...
	g_creatureEvents->playerThink(this, interval);
}
...
void Player::onDeath()
{
	...
	while((uint64_t)(experience - getLostExperience()) < getExpForLv(newLevel))
	{
		if(newLevel > 1)
			newLevel--;
		else
...
uint32_t Player::getGainedExperience(Creature* attacker) const
{
	...
	if(!hasFlag(PlayerFlag_NotGainInFight)){
		if(target != this){
			if(Player* targetPlayer = target->getPlayer()){
				...
				{
					if(!targetPlayer->party || party != targetPlayer->party)
					{
...
void Player::onGainExperience(uint32_t gainExperience)
{
	...
	if(gainExperience > 0){
		//soul regeneration
		if(gainExperience >= (uint32_t)getLevel()){
...
void Player::setSkillsPercents()
{
	...
	percent = std::min(std::max((uint32_t)0, (uint32_t)percent), (uint32_t)100);
	maglevel_percent = percent;
	percent = (100 * (experience - getExpForLv(getLevel()))) / std::max((int32_t)1, (int32_t)((getExpForLv(getLevel() + 1) - getExpForLv(getLevel()))));
	percent = std::min(std::max((uint32_t)0, (uint32_t)percent), (uint32_t)100);
	level_percent = percent;
...
Skulls_t Player::getSkullClient(const Player* player) const
{
	...
	if(skull != SKULL_RED && player->party && player->party == party)
		return SKULL_GREEN;

	if(skull == SKULL_NONE)
	{
		if(player->hasAttacked(this) && (!party || party && party != player->party))
			skull = SKULL_YELLOW;
	}
	return skull;
}
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	if(g_config.getNumber(ConfigManager::KILLS_TO_RED) != 0 && redSkullTicks >= (g_config.getNumber(ConfigManager::KILLS_TO_RED) - 1) * 43200000 && getSkull() != SKULL_RED)
		g_game.changeSkull(this, SKULL_RED);
	else if(g_config.getNumber(ConfigManager::KILLS_TO_BAN) != 0 && redSkullTicks >= (g_config.getNumber(ConfigManager::KILLS_TO_BAN) - 1) * 43200000)
	{
...
{
	return (uint64_t)std::floor(experience * getLostPercent());
}

bool Player::isPromoted()
{
	/*
	 * TODO: Change to in 0.3 if storage will still be used:
		return (bool)player->getStorageValue(STORAGEVALUE_PROMOTION, value);
	 */
...
void Player::manageAccount(const std::string &text)
{
	...
	if(namelockedPlayer != "")
	{
		if(!talkState[1])
		{
			...
			else if(IOLoginData.playerExists(newCharacterName))
		...
		{
			if(IOLoginData.changeName(namelockedPlayer, newCharacterName))
			{
				talkState[1] = true;
				talkState[2] = false;
				uint32_t guid_;
				IOLoginData.getGuidByName(guid_, newCharacterName);
				g_bans.removePlayerNamelock(guid_);
	...
	{
		Account account = IOLoginData.loadAccount(realAccount);
		...
		{
			int32_t result = IOLoginData.deleteCharacter(this);
			switch(result)
			{
				case 0:
		...
		else if(checkText(text, "yes") && talkState[5])
		{
			...
			IOLoginData.changePassword(this);
		...
		else if(checkText(text, "yes") && talkState[9])
		{
			...
			{
				if(!IOLoginData.playerExists(newCharacterName))
				{
					talkState[1] = true;
					for(int8_t i = 2; i <= 12; i++)
						talkState[i] = false;
					if(IOLoginData.createCharacter(this))
		...
		else if(checkText(text, "yes") && talkState[10])
		{
			...
			else
			{
				...
				IOLoginData.setRecoveryKey(this, recoveryKey);
	...
	else
	{
		...
		else if(checkText(text, "yes") && talkState[3])
		{
			if(g_config.getString(ConfigManager::GENERATE_ACCOUNT_NUMBER) == "yes")
			{
				...
				if(g_config.getNumber(ConfigManager::PASSWORD_TYPE) == PASSWORD_TYPE_MD5)
					newPassword = transformToMD5(newPassword);
				IOLoginData.createAccount(this);
		...
		{
			if(!IOLoginData.accountExists(newAccount))
			{
				IOLoginData.createAccount(this);
			...
			{
				msg << "An account number like that already exists, please pick another account number.";
				talkState[4] = true;
				talkState[5] = false;
			}
		}
		...
		else if(talkState[8])
		{
			...
			if(IOLoginData.validRecoveryKey(this) && recoveryKeyAttempt != "0")
			{
				char buffer[45];
				sprintf(buffer, "%s%d", g_config.getString(ConfigManager::SERVER_NAME).c_str(), random_range(100, 999));
				IOLoginData.setNewPassword(this, buffer);
				msg << "That information was right, your new password is: " << buffer << ".";
			}
			else
...
void Player::setGuildLevel(GuildLevel_t newGuildLevel)
{
	...
	setGuildRank(IOGuild.getRankName(guildLevel, guildId));
}

void Player::setGroupId(int32_t newId)
{
	const PlayerGroup* group = IOLoginData.getPlayerGroup(newId);
	if(group)
	{
