...
Creature()
{
	if(!m_scriptInterface){
		...
		m_scriptInterface->loadNpcLib(std::string("data/npc/lib/npc.lua"));
	}
...
bool Npc::canWalkTo(const Position& fromPos, Direction dir)
{
	...
	if(!tile || tile->__queryAdd(0, this, 1, 0) != RET_NOERROR || tile->getHeight() > 0){
		return false;
	}
...
void NpcScript::onCreatureAppear(const Creature* creature)
{
	...
	if(m_scriptInterface->reserveScriptEnv()){
		...
		char desc[35];
		sprintf(desc, "npc %s", m_npc->getName().c_str());
