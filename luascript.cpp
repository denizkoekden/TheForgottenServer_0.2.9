...
int32_t LuaScriptInterface::luaDoTeleportThing(lua_State* L)
{
	...
	if(tmp){
		if(g_game.internalTeleport(tmp,(Position&)pos, pushMove) == RET_NOERROR){
			lua_pushnumber(L, LUA_NO_ERROR);
		}
		else{
			lua_pushnumber(L, LUA_ERROR);
...
int32_t LuaScriptInterface::luaDoSendDistanceShoot(lua_State* L)
{
	...
	Position pos[2];
	uint32_t stackpos[2];
	popPosition(L, pos[1], stackpos[1]);
	popPosition(L, pos[0], stackpos[0]);
	
	ScriptEnviroment* env = getScriptEnv();
	for(int i = 0; i <= 1; i++)
	{
		if(pos[i].x == 0xFFFF)
			pos[i] = env->getRealPos();
	}
		
	g_game.addDistanceEffect(pos[0], pos[1], type);
	lua_pushnumber(L, LUA_NO_ERROR);
	return 1;
}
...
int32_t LuaScriptInterface::luaDoPlayerAddExp(lua_State* L)
{
	...
	if(player)
	{
		if(exp > 0)
		{
			player->addExperience((uint64_t)exp);
			lua_pushnumber(L, LUA_TRUE);
		}
		else
			lua_pushnumber(L, LUA_FALSE);
...
int32_t LuaScriptInterface::luaGetGuildId(lua_State* L)
{
	...
	uint32_t guildId;
	if(IOGuild.getGuildIdByName(guildId, std::string(name))){
...
int32_t LuaScriptInterface::luaDoPlayerAddOutfit(lua_State* L)
{
	...
	int32_t addon = (int32_t)popNumber(L);
	int32_t looktype = (int32_t)popNumber(L);
	uint32_t cid = popNumber(L);
...
int32_t LuaScriptInterface::luaMayNotLogout(lua_State* L)
{
	//mayNotLogout(cid, value)
