...
void Weapon::onUsedAmmo(Player* player, Item* item, Tile* destTile) const
{
	...
	if(it.breakChance == 0 || it.breakChance > 0 && random_range(1, 100) < (int32_t)it.breakChance)
	{
...
bool WeaponDistance::configureEvent(xmlNodePtr p)
{
	...
	if(readXMLInteger(p, "hitChance", intValue))
		hitChance = std::min(std::max((uint32_t)0, (uint32_t)intValue), (uint32_t)100);

	if(readXMLInteger(p, "breakChance", intValue))
		breakChance = std::min(std::max((uint32_t)0, (uint32_t)intValue), (uint32_t)100);
...
bool WeaponDistance::useWeapon(Player* player, Item* item, Creature* target) const
{
	...
	if(random_range(1, 100) < hitChance){
		...
		//miss target
		int32_t dx = random_range(-1, 1);
		int32_t dy = random_range(-1, 1);
