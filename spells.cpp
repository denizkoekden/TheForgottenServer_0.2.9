...
void Spell::postCastSpell(Player* player, bool isConjuringItem) const
{
	if(!player->hasFlag(PlayerFlag_HasNoExhaustion))
	{
		if(exhaustion > 0 && !isConjuringItem)
			player->addExhaustionTicks(exhaustion);
	}
...
bool InstantSpell::HouseKick(const InstantSpell* spell, Creature* creature, const std::string& param)
{
	...
	if(targetPlayer)
	{
		house = getHouseFromPos(targetPlayer);
		if(!house)
			return false;
		house->kickPlayer(creature->getPlayer(), param);
	}
	else
	{
		house = getHouseFromPos(creature);
		if(!house)
			return false;
		house->kickPlayer(creature->getPlayer(), creature->getPlayer()->getName());
	}
	return true;
}
...
{
	if(strcasecmp(functionName.c_str(), "conjureItem") == 0 || strcasecmp(functionName.c_str(), "conjureRune") == 0)
...
bool ConjureSpell::internalConjureItem(Player* player, uint32_t conjureId, uint32_t conjureCount, uint32_t reagentId, slots_t slot)
{
	if(reagentId != 0)
	{
		Item* newItem = g_game.transformItem(player->getInventoryItem(slot), conjureId, conjureCount);
...
bool ConjureSpell::ConjureItem(const ConjureSpell* spell, Creature* creature, const std::string& param)
{
	...
	{
		Item* itemSlotLeft = player->getInventoryItem(SLOT_LEFT);
		if(itemSlotLeft && itemSlotLeft->getID() == spell->getReagentId())
		{
			if(!spell->playerSpellCheck(player))
				return false;
			
			if(internalConjureItem(player, spell->getConjureId(), spell->getConjureCount(), spell->getReagentId(), SLOT_LEFT))
			{
				spell->postCastSpell(player, true);
				result = true;
			}
		}

		Item* itemSlotRight = player->getInventoryItem(SLOT_RIGHT);
		if(itemSlotRight && itemSlotRight->getID() == spell->getReagentId())
		{
			if(!spell->playerSpellCheck(player))
				return false;
	
			if(internalConjureItem(player, spell->getConjureId(), spell->getConjureCount(), spell->getReagentId(), SLOT_RIGHT))
			{
				spell->postCastSpell(player, true);
				result = true;
			}
...
bool RuneSpell::executeCastSpell(Creature* creature, const LuaVariant& var)
{
	...
	if(m_scriptInterface->reserveScriptEnv())
	{
		...
		#ifdef __DEBUG_LUASCRIPTS__
		char desc[45];
		sprintf(desc, "executeCastSpell - %s", creature->getName().c_str());
		env->setEventDesc(desc);
		#endif
