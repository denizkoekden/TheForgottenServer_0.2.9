...
{
	DWORD index;
	index = SendMessage(list, CB_FINDSTRING, 0,(LPARAM)player->getName().c_str());
	if(index != CB_ERR)
...
LRESULT CALLBACK PlayerBox::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch(message) 
	{
		case WM_CREATE:
		{
			...
			SendMessage(permBan, WM_SETFONT, (WPARAM)gui.font, 0);
			SendMessage(kick, WM_SETFONT, (WPARAM)gui.font, 0);
			SendMessage(list, WM_SETFONT, (WPARAM)gui.font, 0);
			SendMessage(online, WM_SETFONT, (WPARAM)gui.font, 0);
			...
			switch(HIWORD(wParam))
			{
				case BN_CLICKED:
				{
					...
					if(player)
					{
						...
						player->setNoLogoutAbility(true);
						...
						if(player)
							player->setNoLogoutAbility(false);
					}
					else
