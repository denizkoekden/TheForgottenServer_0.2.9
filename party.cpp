...
Party::~Party()
{
	...
	std::vector<Player*> tmp_invitations = invitations;
...
void Party::leave(Player* player)
{
	if(members.size() > 0)
	{
		...
		std::vector<Player*>::iterator target = std::find(members.begin(), members.end(), player);
...
void Party::passLeadership(Player* player)
{
	...
	leader = player;
	std::vector<Player*>::iterator target = std::find(members.begin(), members.end(), leader);
	members.erase(target);
...
{
	std::vector<Player*>::iterator target = std::find(invitations.begin(), invitations.end(), player);
