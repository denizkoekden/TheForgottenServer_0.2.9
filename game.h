...
class Game
{
	public:
		...
		void serverSave();
		void saveData(bool kickPlayers);
		bool serverSaveMessage[2];

		#ifdef __RULEVIOLATIONREPORT__
		std::list<Player*> hasRuleViolationsChannelOpen;
		std::vector<Player*> openRuleViolations;
		#endif
		...
		*/
		...
		bool violationWindow(uint32_t playerId, std::string targetPlayerName, int32_t reasonId, int32_t actionId, std::string banComment, bool IPBanishment);
		...
		uint64_t getExperienceStage(int32_t level);
