function onStepOut(cid, item, position, fromPosition)
	local thingPosition = {x = position.x, y = position.y, z = position.z, stackpos = STACKPOS_TOP_MOVEABLE_ITEM_OR_CREATURE}
	local thing = getThingfromPos(thingPosition)
	local fieldPosition = {x = position.x, y = position.y, z = position.z, stackpos = STACKPOS_TOP_FIELD}
