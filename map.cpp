...
bool Map::placeCreature(const Position& centerPos, Creature* creature, bool forceLogin /*=false*/)
{
	...
	if(tile){
		...
		if(forceLogin || ret == RET_NOERROR || ret == RET_PLAYERISNOTINVITED || creature->getName() == "Account Manager")
			foundTile = true;
	}
...
int32_t AStarNodes::getMapWalkCost(const Creature* creature, AStarNode* node, const Tile* neighbourTile)
{
	int32_t cost = 0;
	if(std::abs((int32_t)node->x - neighbourTile->getPosition().x) == std::abs((int32_t)node->y - neighbourTile->getPosition().y)){
