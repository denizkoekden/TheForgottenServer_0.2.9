...
bool ChatChannel::addUser(Player* player)
{
	...
	if(m_id == 0x00)
	{
		if(IOGuild.getMotd(player->getGuildId()).length())
			Scheduler::getScheduler().addEvent(createSchedulerTask(50, boost::bind(&Player::sendChannelMessage, player, "Message of the Day", IOGuild.getMotd(player->getGuildId()), SPEAK_CHANNEL_R1, m_id)));
	}
...
Chat::Chat()
{
	...
	/*
		newChannel = new ChatChannel(0x07, "Not in use");
		if(newChannel)
...
bool Chat::addUserToChannel(Player* player, uint16_t channelId)
{
	...
	if(channelId == 0x01 && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER || channelId == 0x02 && player->getAccountType() < ACCOUNT_TYPE_TUTOR
	#ifdef __RULEVIOLATIONREPORT__
	|| channelId == 0x03 && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER
	#endif
	)
		return false;

	if(player->getAccountType() < ACCOUNT_TYPE_SENIORTUTOR && (channelId == 0x05 && player->getVocationId() == 0 || channelId == 0x06 && player->getVocationId() != 0))
		return false;
...
bool Chat::talkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	...
	if(channelId == 0x00)
	{
		if(text == "!disband" || text.substr(0, 7) == "!invite" || text == "!leave" || text.substr(0, 5) == "!kick" || text.substr(0, 7) == "!revoke" || text.substr(0, 7) == "!demote" || text.substr(0, 8) == "!promote" || text.substr(0, 15) == "!passleadership" || text.substr(0, 5) == "!nick" || text.substr(0, 12) == "!setrankname" || text.substr(0, 8) == "!setmotd" || text == "!cleanmotd" || text == "!commands")
		{
			...
			{
				if(IOGuild.guildExists(player->getGuildId()))
				{
					char buffer[125];
					...
					else if(text.substr(0, 7) == "!invite")
					{
						if(player->getGuildLevel() > GUILDLEVEL_MEMBER)
						{
							if(text.length() > 8)
							{
								...
								else if(IOLoginData.playerExists(param))
								{
									uint32_t guid;
									IOLoginData.getGuidByName(guid, param);
									if(!IOGuild.hasGuild(guid))
									{
										if(!IOGuild.isInvitedToGuild(guid, player->getGuildId()))
										{
											if(IOGuild.guildExists(player->getGuildId()))
											{
												IOGuild.invitePlayerToGuild(guid, player->getGuildId());
								...
								else if(IOLoginData.playerExists(param))
								{
									...
									if(IOGuild.isInvitedToGuild(guid, player->getGuildId()))
									{
										if(IOGuild.guildExists(player->getGuildId()))
										{
											...
											guildChannel->talk(player, SPEAK_CHANNEL_R2, buffer);
											IOGuild.revokeGuildInvite(guid, player->getGuildId());
										}
										else
					...
					else if(text.substr(0, 8) == "!promote" || text.substr(0, 7) == "!demote" || text.substr(0, 15) == "!passleadership" || text.substr(0, 5) == "!kick")
					{
						if(player->getGuildLevel() == GUILDLEVEL_LEADER)
						{
							...
							if(paramPlayer)
							{
								...
								{
									if(IOGuild.guildExists(paramPlayer->getGuildId()))
									{
							...
							else if(IOLoginData.playerExists(param))
							{
								...
								if(IOGuild.hasGuild(guid))
								{
									if(player->getGuildId() == IOGuild.getGuildId(guid))
									{
										if(text[2] == 'r')
										{
											if(IOGuild.getGuildLevel(guid) == GUILDLEVEL_MEMBER)
											{
												if(IOLoginData.isPremium(guid))
												{
													IOGuild.setGuildLevel(guid, GUILDLEVEL_VICE);
													sprintf(buffer, "%s has promoted %s to %s.", player->getName().c_str(), param.c_str(), IOGuild.getRankName(IOGuild.getGuildLevel(guid), IOGuild.getGuildId(guid)).c_str());
													guildChannel->talk(player, SPEAK_CHANNEL_R2, buffer);
												}
												else
													player->sendCancel("A player with that name does not have a premium account.");
										...
										else if(text[2] == 'e')
										{
											...
											{
												IOGuild.setGuildLevel(guid, GUILDLEVEL_MEMBER);
												sprintf(buffer, "%s has demoted %s to %s.", player->getName().c_str(), param.c_str(), IOGuild.getRankName(IOGuild.getGuildLevel(guid), IOGuild.getGuildId(guid)).c_str());
												guildChannel->talk(player, SPEAK_CHANNEL_R2, buffer);
											}
											else
												player->sendCancel("You can only demote Vice-Leaders to Members.");
										...
										else
										{
											...
											guildChannel->talk(player, SPEAK_CHANNEL_R2, buffer);
											IOLoginData.resetGuildInformation(guid);
										}
									}
								}
								else
					...
					else if(text.substr(0, 5) == "!nick")
					{
						if(text.length() > 7)
						{
							...
							{
								uint32_t guid;
								IOLoginData.getGuidByName(guid, (std::string&)param);
								if(IOGuild.hasGuild(guid))
								{
									if(param2.length() > 3)
									{
										if(param2.length() < 15)
										{
											if(isValidName(param2))
											{
												if(IOGuild.guildExists(guid))
												{
													if(player->getGuildId() == IOGuild.getGuildId(guid))
													{
														if(IOGuild.getGuildLevel(guid) < player->getGuildLevel())
														{
															IOGuild.setGuildNick(guid, param2);
															sprintf(buffer, "%s has set the guildnick of %s to \"%s\".", player->getName().c_str(), param.c_str(), param2.c_str());
															guildChannel->talk(player, SPEAK_CHANNEL_R2, buffer);
														}
														else
															player->sendCancel("You may only change the guild nick of players that have a lower rank than you.");
							...
							if(player->getGuildLevel() == GUILDLEVEL_LEADER)
							{
								if(param2.length() > 3)
								{
									if(param2.length() < 21)
									{
										if(isValidName(param2))
										{
											if(IOGuild.rankNameExists(param1, player->getGuildId()))
											{
												if(!IOGuild.rankNameExists(param2, player->getGuildId()))
												{
													IOGuild.changeRankName(param1, param2, player->getGuildId());
						...
						if(player->getGuildLevel() == GUILDLEVEL_LEADER)
						{
							if(text.length() > 9)
							{
								...
								if(param.length() > 3)
								{
									...
									{
										char _buffer[310];
										IOGuild.setMotd(player->getGuildId(), param);
					...
					else if(text == "!cleanmotd")
					{
						if(player->getGuildLevel() == GUILDLEVEL_LEADER)
						{
							IOGuild.setMotd(player->getGuildId(), "");
...
ChannelList Chat::getChannelList(Player* player)
{
	...
	for(itn = m_normalChannels.begin(); itn != m_normalChannels.end(); ++itn)
	{
		...
		if(itn->first == 0x01 && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER || itn->first == 0x02 && player->getAccountType() < ACCOUNT_TYPE_TUTOR
		#ifdef __RULEVIOLATIONREPORT__
		|| itn->first == 0x03 && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER
		#endif
		)
			skip = true;

		if(player->getAccountType() < ACCOUNT_TYPE_SENIORTUTOR && (itn->first == 0x06 && player->getVocationId() != 0  || itn->first == 0x05 && player->getVocationId() == 0))
			skip = true;
