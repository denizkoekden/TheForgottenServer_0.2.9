...
class ItemAttributes
{
	public:
		...
		void setDate(time_t n) {setIntAttr(ATTR_ITEM_WRITTENDATE, n);}
		void resetDate() {removeAttribute(ATTR_ITEM_WRITTENDATE);}
		time_t getDate() const {return (time_t)getIntAttr(ATTR_ITEM_WRITTENDATE);}
	
		void setSleeper(uint32_t n){setIntAttr(ATTR_ITEM_SLEEPER, n);}
		void resetSleeper()
		{
			removeAttribute(ATTR_ITEM_SLEEPER);
			resetSpecialDescription();
		}
		uint32_t getSleeper() const {return getIntAttr(ATTR_ITEM_SLEEPER);}
...
class Item : virtual public Thing, public ItemAttributes
{
	public:
		...
		void transformTakenBedIntoFree(Position position);
		void transformFreeBedIntoTaken(Position position);
