...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target)
{
	if(attacker)
	{
		...
		if(g_game.getWorldType() == WORLD_TYPE_NO_PVP)
		{
			...
			if(Player* attackerPlayer = attacker->getPlayer())
			{
				if(target && target->getPlayer() && attacker->getPlayer() && (target->getPlayer()->getLevel() < g_config.getNumber(ConfigManager::PROTECTION_LEVEL) || attackerPlayer->getLevel() < g_config.getNumber(ConfigManager::PROTECTION_LEVEL)))
					return RET_YOUMAYNOTATTACKTHISPLAYER;
			}
		}
	}
