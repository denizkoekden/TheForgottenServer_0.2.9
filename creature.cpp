...
void Creature::onDeath()
{
	...
	if(eventDeath)
		eventDeath->executeOnDeath(this, corpse, lastHitCreature);
}
...
uint32_t Creature::getGainedExperience(Creature* attacker) const
{
...
void Creature::onAttackedCreatureKilled(Creature* target)
{
	if(target != this)
	{
		uint32_t gainedExperience = target->getGainedExperience(this);
...
void Creature::onGainExperience(uint32_t gainExperience)
{
	if(gainExperience > 0)
	{
		if(getMaster())
